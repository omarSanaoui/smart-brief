...
export default function MyBriefs() {
  ...
  const handleOpenBrief = (brief: Brief) => setSelectedBriefId(brief.id);
  ...
  return (
    <section className="font-poppins text-white sm:min-h-[calc(100vh-80px)] pt-10 sm:pt-16 pb-10 sm:pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto overflow-x-hidden">
      ...
      <div className="flex justify-between items-end mb-16 relative">
        <div className="absolute -top-10 -left-10 w-32 h-32 bg-sbteal/10 blur-[50px] rounded-full" />
        <div>
          <h1 className="text-[36px] md:text-[52px] font-black tracking-widest uppercase leading-tight">
            {t("myBriefs.title").split(" ")[0]} <span className="text-sbteal">{t("myBriefs.title").split(" ").slice(1).join(" ")}</span>
          </h1>
          <p className="text-[#64748B] text-sm mt-3 font-medium tracking-wide">{t("myBriefs.subtitle")}</p>
        </div>
        <button onClick={() => navigate('/new-project')}
          ...
          <Plus size={16} className="group-hover:rotate-90 transition-transform" /> {t("myBriefs.newProject")}
        </button>
      </div>
      ...
      <button onClick={() => navigate('/new-project')}
        ...
        <Plus size={16} /> {t("myBriefs.newProject")}
      </button>
      ...
      {loading && briefs.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-80 gap-6">
          ...
          <span className="text-[#64748B] font-bold text-xs uppercase tracking-widest animate-pulse">{t("myBriefs.loading")}</span>
        </div>
      ) : briefs.length === 0 ? (
        <div className="bg-[#1A2238]/50 border border-[#2E3A5C] backdrop-blur-sm rounded-[32px] p-16 flex flex-col items-center justify-center text-center shadow-2xl relative overflow-hidden">
          <div className="absolute -top-20 -right-20 w-60 h-60 bg-sbpurple/5 blur-[80px] rounded-full" />
          <div className="bg-[#2D3652] p-6 rounded-3xl text-white/10 mb-8 border border-[#2E3A5C]">
            <FileText size={48} />
          </div>
          <h3 className="text-2xl font-black mb-3 uppercase tracking-wider">{t("myBriefs.empty.title")}</h3>
          <p className="text-[#64748B] mb-10 text-sm max-w-sm font-medium">{t("myBriefs.empty.subtitle")}</p>
          <button onClick={() => navigate('/new-project')}
            ...
            {t("myBriefs.empty.cta")}
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {briefs.map((brief) => (
            <BriefCard key={brief.id} brief={brief} onClick={() => handleOpenBrief(brief)} />
          ))}
        </div>
      )}
    </section>
