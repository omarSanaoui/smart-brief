...
export default function BriefsDashboard() {
  ...
  // Different titles based on roles
  const title = isAdmin ? "GLOBAL" : "ASSIGNED";
  const subtitle = isAdmin ? "Operational oversight of all active project documentation." : "Direct access to your assigned professional briefs.";
    ...
    <section className="font-poppins text-white min-h-[calc(100vh-80px)] pt-16 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8 border-b border-[#2E3A5C]/40 pb-10">
        <div>
          <h1 className="text-[36px] md:text-[52px] font-black tracking-widest uppercase leading-tight">
            {title} <span className="text-sbteal">DASHBOARD</span>
          </h1>
          ...
          <div className="flex gap-4 w-full md:w-auto">
             <div className="flex-1 md:flex-none bg-[#1A2238]/60 px-6 py-3 rounded-2xl flex items-center gap-3 border border-[#2E3A5C]/40">
               ...
               <div className="flex flex-col">
                 <span className="text-[9px] text-[#64748B] uppercase font-black tracking-widest leading-none mb-1">Total Docs</span>
             ...
             <div className="flex-1 md:flex-none bg-[#1A2238]/60 px-6 py-3 rounded-2xl flex items-center gap-3 border border-[#2E3A5C]/40">
               ...
               <div className="flex flex-col">
                 <span className="text-[9px] text-[#64748B] uppercase font-black tracking-widest leading-none mb-1">Pending</span>
      ...
      <div className="flex justify-between items-center mb-8">
         <h2 className="text-[#64748B] font-black text-[11px] uppercase tracking-[0.3em] flex items-center gap-2">
            <Filter size={14}/> Active Documentation
         </h2>
      </div>
      ...
      {loading && briefs.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-80 gap-6">
          ...
          <span className="text-[#64748B] font-bold text-xs uppercase tracking-widest animate-pulse">Fetching records...</span>
        </div>
      ) : briefs.length === 0 ? (
        <div className="bg-[#1A2238]/50 border border-[#2E3A5C] backdrop-blur-sm rounded-[32px] p-16 flex flex-col items-center justify-center text-center shadow-2xl">
          ...
          <h3 className="text-xl font-black uppercase tracking-wider mb-2">No Records Available</h3>
          <p className="text-[#64748B] text-sm font-medium">There are currently no professional briefs logged in this sector.</p>
        </div>
      ) : (
